...
int main(int argc, char *argv[]){
    ...
    while((entry = readdir(dir)) != NULL){  // itera su tutte le entry della directory
        ...
        if(S_ISREG(st.st_mode)){         // processa solo file regolari (no dir, no symlink)
            ...
            DIR *dir2 = opendir(".");        // riapre la directory per il loop interno
            if(dir2 == NULL){
                perror("opendir");
                free(buffer);
                fclose(fp);
            ...
            struct dirent *entry2;           // entry del loop interno (file candidato target)
            int flag = 0;                    // diventa 1 se buffer contiene almeno un pathname della directory
            while((entry2 = readdir(dir2)) != NULL){   // scorre tutti i file della directory
                if(strcmp(entry2->d_name, ".") == 0 || strcmp(entry2->d_name, "..") == 0){
                    continue;               // salta . e ..
                }

                char target_path[PATH_MAX];
                // costruisce il pathname completo del file target (es. /tmp/mydir/altro)
                snprintf(target_path, sizeof(target_path), "%s/%s", cwd, entry2->d_name);
                if(strstr(buffer, target_path) != NULL){  // il contenuto contiene quel pathname?
                    flag = 1;   // sì: il file è dir-ricorsivo
                    break;      // basta trovarne uno, interrompe il loop interno
                }
            }
            closedir(dir2);     // chiude il secondo handle della directory

            // se buffer contiene il pathname completo di almeno un file della directory: è dir-ricorsivo
            if(flag){
